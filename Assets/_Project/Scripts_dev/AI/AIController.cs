...
namespace _Project.Scripts_dev.AI
{
    public class AIController : MonoBehaviour
    {
        ...
        [SerializeField] private bool famer, customer,shipper;

        public Cart cart;
        [SerializeField] private string tagFarm;
        [SerializeField] private GameObject waitingSpot;
        [SerializeField] private GameObject bongBong;
        [SerializeField] private GameObject tool;
        [SerializeField] private Sprite[] wishes;
        [SerializeField] private Image customerWishPop;
        ...
        private void Update()
        {
            ...
            if (customer) {
                ...
                if (toCashier)
                {//sua cho nay
                    if (_gameManager.some1Taking==0&&!choosing)
                    {
        ...
        private void FindFarmsThatNeedsWorker()
        {
        ...
        private GameObject ChooseShop(bool reChoose=false)
        {
            ...
            if (_gameManager.shops.Count == 0||goHome) return null;
       
            GameObject shop = _gameManager.shops[Random.Range(0, _gameManager.shops.Count)];
            if(reChoose==false)
                while (shopsPassed.Contains(shop) && _gameManager.shops.Count != shopsPassed.Count)
                {
            ...
            if (_gameManager.shops.Count == shopsPassed.Count) return null;
        ...
        private void CustomerGrab(Shelf shelf)
        {
        ...
        private IEnumerator WaitForShop(Shelf shelf,bool first =false)
        {
            ...
            if (stayTimer >= 20 )
            {
                ...
                if (newShop == null || shopsPassed.Count == 3) 
                { 
                    stayTimer = 21;
                    StopAllCoroutines(); 
                }

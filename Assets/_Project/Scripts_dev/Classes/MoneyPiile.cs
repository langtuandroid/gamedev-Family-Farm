...
namespace _Project.Scripts_dev.Classes
{
    public class MoneyPiile : MonoBehaviour
    {
        ...
        private void UpdateStockLooks()
        {
            ...
            if (Mathf.Ceil(currentQuantity / _gameManager.moneyPerPack) < pos.Length)
                DelayDeative();
        }
        public void UpdateNextPos(bool add)
        {
            ...
            if (add)
                nextIndex++;
            else if (Mathf.Ceil(currentQuantity / _gameManager.moneyPerPack) <= pos.Length)
            {
        ...
        private IEnumerator DelayActive()
        {
            ...
            for (int i = 0; i < (currentQuantity / _gameManager.moneyPerPack > pos.Length ? pos.Length :Mathf.Ceil(currentQuantity / _gameManager.moneyPerPack)); i++)
            {
        ...
        {
            for (int i =(int) (Mathf.Ceil(currentQuantity / _gameManager.moneyPerPack)); i < pos.Length; i++)
            {
